...
namespace Elite
{
	...
	template<class T_NodeType, class T_ConnectionType>
	inline void IGraph<T_NodeType, T_ConnectionType>::RemoveNode(int node)
	{
		...
		assert(node < (int)m_Nodes.size() && "<Graph::RemoveNode>: invalid node index");
		...
		if (!m_IsDirectionalGraph)
		{
			...
			for (auto currentConnection = m_Connections[node].begin();
				currentConnection != m_Connections[node].end();
				++currentConnection)
			{
	...
	template<class T_NodeType, class T_ConnectionType>
	inline Elite::Color IGraph<T_NodeType, T_ConnectionType>::GetNodeColor(T_NodeType* pNode) const
	{
	...
	template<>
	inline Elite::Color IGraph<GraphNode2D, GraphConnection2D>::GetNodeColor(GraphNode2D* pNode) const
	{
	...
	template<class T_NodeType, class T_ConnectionType>
	inline Elite::Color IGraph<T_NodeType, T_ConnectionType>::GetConnectionColor(T_ConnectionType* pNode) const
	{
		return GraphConnection::DEFAULT_CONNECTION_COLOR;
	}

	// Template specialization
	template<>
	inline Elite::Color IGraph<GraphNode2D, GraphConnection2D>::GetConnectionColor(GraphConnection2D* pConnection) const
	{
		return pConnection->GetColor();
	}
